...
import (
	...
	"fmt"
	"log"
	"newsSearchEngine/engine/arrays"
	"newsSearchEngine/engine/model"
	"newsSearchEngine/engine/pagination"
	"newsSearchEngine/engine/sorts"
	"newsSearchEngine/engine/storage"
...
type Engine struct {
	...
	//Storage
	invertedIndexStorages []*storage.LeveldbStorage // Key,Value={id,[]words} Refletion: Id - Keywords
	positiveIndexStorages []*storage.LeveldbStorage //ID和key映射，用于计算相关度，一个id 对应多个key，正排索引
	docStorages           []*storage.LeveldbStorage //Document storage shards

	//Utils
	sync.Mutex
	sync.WaitGroup

	addDocumentWorkerChan []chan *model.IndexDoc //Chan to add index
	IsDebug               bool                   //FOR DEBUG
	Tokenizer             *tokenizer.Tokenizer   //tokenizer
	DatabaseName          string                 //DB name

	//Basic info
	Shard     int   //shards num
	Timeout   int64 //timeout, count in sec
	BufferNum int   //buf num for shard

	docCount int64 //doc total num
}
...
func (e *Engine) Init() {
	...
	//-1 normalize as process proceed
	e.docCount = -1
...
// Auto Save index, check every 10s
...
func (e *Engine) MultiSearch(request *model.SearchRequest) (*model.SearchResult, error) {
	...
	t, err := utils.ExecTimeWithError(func() error {
		...
		if pager.PageCount != 0 {
			...
			if ScoreExp != "" {
				// 生成计算表达式
				exp, err := govaluate.NewEvaluableExpression(ScoreExp)
				if err != nil {
					return err
				}
				...
				for i, doc := range result.Documents {
					...
					if err != nil {
						log.Printf("表达式执行'%v'错误: %v 值内容: %v", ScoreExp, err, parameters)
					} else {
...
func (e *Engine) GetDocumentCount() int64 {
	if e.docCount == -1 {
		...
		wg := sync.WaitGroup{}
		wg.Add(e.Shard)
		//not acculate because without lock protection
		for i := 0; i < e.Shard; i++ {
			go func(i int) {
...
// getShard calculate doc location of file
